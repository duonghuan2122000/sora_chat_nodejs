    ...
    <div class="flex-none w-3/4 border-l border-gray-200">
      <div class="flex flex-col h-full">
        ...
        <div class="flex-1 p-4 overflow-y-auto">
          <ClientOnly>
            <template v-for="message in messages" v-if="ready">
              <!-- Dòng tin nhắn -->
              <div
                v-if="!checkCurrentUserSent(message.sender.user_id)"
                class="flex flex-row flex-wrap items-end gap-3 px-4 py-2"
              >
                <div class="flex-none">
                  <img
                    src="https://cdn-media.sforum.vn/storage/app/media/anh-dep-82.jpg"
...
const { user, fetchUser, isLoggedIn, ready } = useAuth();
...
const checkCurrentUserSent = (sentUserId) => {
  return user.value?.id === sentUserId;
};
